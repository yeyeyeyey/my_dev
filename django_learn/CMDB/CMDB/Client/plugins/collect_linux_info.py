...
def collect():
    filter_keys = ['Manufacturer','Serial Number','Product Name','UUID',
                   'Wake-up Type']
    raw_data ={}
    for key in filter_keys:
        try:
            res = subprocess.Popen("sudo dmidcede -t system|grep '%s'"%key,
    ...
    data['wake_uo_type'] = raw_data['Wake-up Type']
...
def get_cpu_info():
    ...
    raw_data = {
        ...
        'cpu_core_count':" %s |grep 'cpu cores' |awk -F:'{SUM +=$2} END {"
                         "print SUM}'"%raw_cmd,
    }
...
def get_ram_info():
    ...
    :return:
    """
    raw_data = subprocess.Popen("sudo dmidecode -t memory",
                                stdout=subprocess.PIPE, shell=True)
    ...
    raw_total_size = subprocess.Popen("cat /proc/meminfo|grep MemTotal ",
                                      stdout=subprocess.PIPE, shell=True)
...
def get_nic_info():
    ...
    :return:
    """
    raw_data = subprocess.Popen("ifconfig -a", stdout=subprocess.PIPE,
                                shell=True)
...
def get_disk_info():
    ...
    :return:
    """
    raw_data = subprocess.Popen("sudo hdparm -i /dev/sda | grep Model",
                                stdout=subprocess.PIPE, shell=True)
    ...
    size_data = subprocess.Popen("sudo fdisk -l /dev/sda | grep Disk|head -1",
                                 stdout=subprocess.PIPE, shell=True)
